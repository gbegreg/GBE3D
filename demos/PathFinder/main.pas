    ...
    for x := 0 to lGrille -1 do begin
      if indice >= 1 then begin
        with rectangleModele.Clone(nil) as TRectangle do begin
          ...
          position.x := x * width;
          position.y := y * height;
          tag := indice;  // le tag va contenir l'indice de la case, ce qui permettra d'avoir ensuite les coordonnées X et Y de la case
...
begin
  ...
  if PathFinder.RechercherChemin then begin dessinerResultat; // Si on a trouvé un chemin, on le dessine
